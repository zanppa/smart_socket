...
// Forward declaration of a quick hack
void txrxDoneISRf(int gpio_n, int level, uint32_t tick, void *modemptr);


void do_debug(LoRa_ctl *modem)
{
	printf("Defaults:\r\n");
	printf("RF F HI : 0x%x  (0x6C)\r\n", lora_reg_read_byte(modem->spid, REG_FR_MSB));
	printf("RF F MID: 0x%x  (0x80)\r\n", lora_reg_read_byte(modem->spid, REG_FR_DIM));
	printf("RF F LOW: 0x%x  (0x00)\r\n", lora_reg_read_byte(modem->spid, REG_FR_LSB));
}
...
int LoRa_begin(LoRa_ctl *modem){
    ...
    // Debug to see if communication is working at all...
    do_debug(modem);
    ...
    // Set the combined interrupt and never touch it again
    lora_set_txdone_dioISR(modem->eth.dio0GpioN, txrxDoneISRf, modem);
...
void txrxDoneISRf(int gpio_n, int level, uint32_t tick, void *modemptr) {
    ...
    if(modem->is_rx) rxDoneISRf(gpio_n, level, tick, modemptr);
    else txDoneISRf(gpio_n, level, tick, modemptr);
}
