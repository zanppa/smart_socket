 ...
 *   Changed GPIO, pin and SPI type definitions to proper typedefs from std lib
 *   Added packet RSSI, packet SNR, CRC OK functions
 *   Added above mentioned variables to the module struct
 *   Removed declaration of internal functions
 */
...
#define SX1278_LORA_CR5		0
#define SX1278_LORA_CR6		1
#define SX1278_LORA_CR7		2
#define SX1278_LORA_CR8		3

static const uint8_t SX1278_LoraErrorCodingRate[4] = { 1, // 4/5
		2,	// 4/6
		3,	// 4/7
		4	// 4/8
		};
...
typedef struct {
	...
	uint8_t crc;
	uint8_t rssi;	// In low freq mode: packet strength [dBm] = -164 + RSSI; in HF mode -157 + RSSI
					// If SNR < 0, then strength [dBm] = -164 + RSSI + SNR * 0.25; in HF mode -157
	int8_t snr;	// SNR is signed value, SNR [dB] = value / 4

	signed char temperature;	// Latest temperature reading in degC
} SX1278_t;
...
// Switch to transmit mode and send a packet
...
// Check if packets received and copy to internal memory
uint8_t SX1278_available(SX1278_t * module);

// Read from internal memory to output buffer (call available first)
uint8_t SX1278_read(SX1278_t * module, uint8_t *rxBuf, uint8_t length);

uint8_t SX1278_RSSI_LoRa(SX1278_t * module);
uint8_t SX1278_PktRSSI_LoRa(SX1278_t * module);
int8_t SX1278_PktSNR_LoRa(SX1278_t * module);
uint8_t SX1278_PktCRCOk_LoRa(SX1278_t * module);
